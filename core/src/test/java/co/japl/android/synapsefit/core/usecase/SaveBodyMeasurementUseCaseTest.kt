...
class SaveBodyMeasurementUseCaseTest {
    ...
    @Test
    fun `when chest measurement is negative, return failure result`() = runTest {
    ...
    @Test
    fun `when waist measurement is negative, return failure result`() = runTest {
    ...
    @Test
    fun `when parameters are valid, save measurement and return success`() = runTest {
        ...
        coVerify(exactly = 1) {
            repositoryPort.saveMeasurement(match {
                it.weightKg == 75.5 && it.chestCm == 100.0 && it.waistCm == 82.0 && it.notes == "Morning measurement"
            })
        }
    }
}
