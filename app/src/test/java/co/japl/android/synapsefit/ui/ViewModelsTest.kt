...
class ViewModelsTest {
    ...
    fun workoutHistoryViewModel_groupsMultipleExercisesOnSameDay() =
        runTest {
            ...
            every { mockLogPort.getWorkoutSummaries() } returns flowOf(summaries)

            val useCase = GetWorkoutHistorySummaryUseCase(mockLogPort)
            val viewModel = WorkoutHistoryViewModel(getWorkoutHistorySummaryUseCase = useCase)
    ...
    fun workoutHistoryViewModel_loadsAllWorkoutSessionsAcrossMultipleDays() =
        runTest {
            ...
            val summaries =
                (1..10).map { index ->
                    WorkoutSummaryItem(
                        sessionId = "plan-1_1_2023-10-0$index",
                        sessionTitle = "Plan 1 - Día $index",
                        avgWeightKg = 50.0,
                        totalDurationSeconds = 600L,
                        dateIso = "2023-10-0$index",
                        day = index,
